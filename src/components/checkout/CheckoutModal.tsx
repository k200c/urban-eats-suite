...
export function CheckoutModal({ open, onOpenChange, onSuccess }: CheckoutModalProps) {
  ...
  const canPayCash = tenderedValue >= total;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-card border-border">
        <DialogHeader>
          <DialogTitle className="font-heading text-2xl text-center">CHECKOUT</DialogTitle>
        </DialogHeader>
                  ...
                  >
                    {canPayCash ? (
                      <>
                        ...
                        <p className="font-heading text-4xl text-destructive mt-1">
                          €{changeDue.toFixed(2)}
                        </p>
                      </>
                    ) : (
